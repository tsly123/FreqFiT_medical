...
class FreqFit_LoRAWrapper_DINO(BaseWrapper):
    def __init__(self, model=VisionTransformer, freqfit_config='freqfit', lora_targets=None, *args, **kwargs) -> None:
        ...
        self.model = get_peft_model(self.model,
                                    LoraConfig(target_modules=lora_targets))
        self.feat_dim = 768

        for name, param in self.model.named_parameters():
            if "lora" in name or 'filter' in name or 'scale' in name or 'shift' in name:# or "pos_embed" in name:
