...
class BaseTrainer(object):
    ...
    def log_model_info(self, verbose=False):
        """Logs model info"""
        if verbose:
            self.logger.info(f"Classification Model:\n{self.model}")
        model_total_params = sum(p.numel() for p in self.model.parameters())
        model_grad_params = sum(
            p.numel() for p in self.model.parameters() if p.requires_grad)
        self.logger.info("Total Parameters: {0}\t Gradient Parameters: {1}".format(
            model_total_params, model_grad_params))
        self.logger.info("tuned percent:%.3f" % (model_grad_params / model_total_params * 100))
