...
class CLSTrainer(BaseTrainer):
    ...
    def train(self, train_dataloader, val_dataloader=None):
        ...
        while self.epoch < self.total_epochs:
            ...
            if self.search_epoch is not None:
                if self.epoch >= self.search_epoch:
                    break
            ...
            if val_dataloader is not None:
                self.evaluate(val_dataloader, split="Val")
    ...
    def evaluate(self, dataloader, save_path=None, split="Test"):
        self.model.eval()
        ...
        self.logger.info("----------------{}: eva epoch {}------------------".format(split, self.epoch))
        self.logger.info(
            "{}: {}".format(split,
                ", ".join("{}: {}".format(k, v) for k, v in organized_metrics.items()),
            )
        )
        self.logger.info("-----------------{}: meta info-------------------".format(split))
        self.logger.info(
            "{}: overall metrics: {}".format(split,
                ", ".join("{}: {}".format(k, v) for k, v in overall_metrics.items()),
            )
        )
        self.logger.info(
            "{}: subgroup metrics: {}".format(split,
